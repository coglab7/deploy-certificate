...
- `DOMAINS`: 要设置的域名，例如要设置 example.com，这里填写的就是 example.com；如果要设置二级泛域名解析，则填写比如 *.example.com。多个域名用英文逗号隔开。
- `ALIYUN_CDN_DOMAINS`: 要设置的CDN域名，例如要设置 cdn.example.com，这里填写的就是 cdn.example.com。多个域名用英文逗号隔开。
- `EMAIL`:  证书过期时提醒的邮件
